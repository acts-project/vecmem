...
void jaggedVectorKnownHtoDCopy(::benchmark::State& state) {
    ...
    // Perform the copy benchmark.
    for (auto _ : state) {
        hip_copy(source_data, dest, copy::type::host_to_device);
    }
}
// Set up the benchmark.
...
void jaggedVectorKnownDtoHCopy(::benchmark::State& state) {
    ...
    // Perform the copy benchmark.
    for (auto _ : state) {
        hip_copy(source, dest_data, copy::type::device_to_host);
    }
}
// Set up the benchmark.

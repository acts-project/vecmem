...
/// CUDA stream to use in the benchmark(s).
static cuda::stream_wrapper stream;
...
static cuda::host_memory_resource cuda_host_mr;

/// Device memory resource to use in the benchmark(s).
static cuda::device_memory_resource device_mr;
...
static cuda::copy device_copy;
/// The asynchronous device copy object to use in the benchmark(s).
static cuda::async_copy async_device_copy{stream};
...
//
EDM_COPY_BENCHMARKS(cuda_pageable_sync, host_mr, device_mr, host_copy,
                    device_copy);
EDM_COPY_BENCHMARKS(cuda_pageable_async, host_mr, device_mr, host_copy,
                    async_device_copy);
EDM_COPY_BENCHMARKS(cuda_pinned_sync, cuda_host_mr, device_mr, host_copy,
                    device_copy);
EDM_COPY_BENCHMARKS(cuda_pinned_async, cuda_host_mr, device_mr, host_copy,
                    async_device_copy);

...
arena::arena(std::size_t initial_size, std::size_t maximum_size, memory_resource& mm) 
...
block arena::expand_arena(std::size_t size) {
  std::pair<std::set<block>::iterator,bool> ret = free_blocks_.insert({mm_.allocate(size), size});
  current_size_ += size;
  return *(ret.first);
}

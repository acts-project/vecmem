...
public:
     ...
     */
    VECMEM_CORE_EXPORT explicit coalescing_memory_resource(
        std::vector<std::reference_wrapper<memory_resource>>&& upstreams);
    /// Move constructor
    VECMEM_CORE_EXPORT
    coalescing_memory_resource(coalescing_memory_resource&& parent) noexcept;
    /// Disallow copying the memory resource
    coalescing_memory_resource(const coalescing_memory_resource&) = delete;

    /// Destructor
    VECMEM_CORE_EXPORT
    ~coalescing_memory_resource() override;

    /// Move assignment operator
    VECMEM_CORE_EXPORT
    coalescing_memory_resource& operator=(
        coalescing_memory_resource&& rhs) noexcept;
    /// Disallow copying the memory resource

...
public:
     ...
     */
    VECMEM_CORE_EXPORT explicit debug_memory_resource(
        memory_resource& upstream);
    /// Move constructor
    VECMEM_CORE_EXPORT
    debug_memory_resource(debug_memory_resource&& parent) noexcept;
    /// Disallow copying the memory resource
    debug_memory_resource(const debug_memory_resource&) = delete;

    /// Destructor
    VECMEM_CORE_EXPORT
    ~debug_memory_resource() override;

    /// Move assignment operator
    VECMEM_CORE_EXPORT
    debug_memory_resource& operator=(debug_memory_resource&& rhs) noexcept;
    /// Disallow copying the memory resource
